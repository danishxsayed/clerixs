...
export function PatientQueueCard({ entry, isFirst }: { entry: any, isFirst: boolean }) {
  ...
  return (
    <div className={cn(
      ...
      isFirst && entry.status === 'waiting' && "border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.15)] ring-1 ring-emerald-500/20 scale-[1.02]",
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            {entry.is_walkin && (
              <Badge variant="secondary" className="text-[10px] py-0 h-4 bg-orange-100 text-orange-700 border-orange-200 uppercase font-bold tracking-tighter">
                Walk-in
              </Badge>
            )}
            {isFirst && entry.status === 'waiting' && (
              <Badge variant="default" className="text-[10px] py-0 h-4 bg-emerald-500 uppercase font-bold tracking-tighter">
                Next
              </Badge>
            )}
            {isInConsultation && (
        ...
        <div className="flex flex-col gap-1 shrink-0">
          ...
          {entry.status === 'waiting' && isFirst && (
             <Button 
               size="sm" 
               className="h-7 px-3 text-[11px] bg-emerald-600 hover:bg-emerald-700 shadow-sm animate-pulse-subtle"

...
export function TreatmentEditForm({ treatment, patients }: { treatment: any, patients: {id: string, full_name: string}[] }) {
  ...
  const onSubmit = (data: TreatmentFormValues) => {
    startTransition(async () => {
      try {
        const result = await updateTreatment(treatment.id, data);
        if (result?.error) {
